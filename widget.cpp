    ...
    , ui(new Ui::Widget)
{
    ...
    // connect(ui->pushButtonPrev, &QToolButton::clicked, m_player, &QMediaPlaylist::previous);
   //  connect(ui->pushButtonNext, &QToolButton::clicked, m_player, &QMediaPlaylist::next);
...
//     connect(ui->tableViewPlaylist, &QTableView::doubleClicked, m_playlist_model, &QMediaPlaylist::setCurrentIndex,
        ...
        [this]()
{
    ...
    {
//        QMessageBox mb(QMessageBox::Icon::Information, m_playlist->se);
       ...
       // qDebug()<<buffer << "\n";
        ...
        if(buffer.split(' ')[0] == "FILE")
        {
           // flac_file = buffer.remove(0, strlen("FILE")+2);
            ...
            items.append(new QStandardItem(flac_file));
            m_playlist_model->appendRow(items);
...
//       m_player->setMedia(QUrl::fromLocalFile(file));
       ...
       QStringList files = QFileDialog::getOpenFileNames
               (
                   ...
                   "Open files",
                    "D:\QT\Source\Repos\Qt\build-MediaPlayer-Desktop_Qt_5_12_12_MSVC2015_64bit-Profile",
                   "Audio files(*.mp3 *.flac);; MP-3(*.mp3);;Flac(*.flac);;Playlist(*.m3u *.cue);;M3U;; CUE(*.cue)"
                   );
       if(files.size()>1)
       {
       ...
       else
       {
           ...
           if(files.last().split('.').last() == "cue")load_playlist(files.last());
       }
}
...
void Widget::on_position_changed(qint64 position)
{
    ...
    QTime qt_position = QTime::fromMSecsSinceStartOfDay(position);
     ui->labelDuration->setText(QString("Duration: ").append(qt_position.toString("mm:ss")));
}
...
void Widget::on_checkBoxLoop_stateChanged(int arg1)
{
    ...
    if(ui->checkBoxLoop->checkState())m_playlist->setPlaybackMode(QMediaPlaylist::PlaybackMode::Random);
}
